...
class Status(object):
    ...
    def print_ssh_status(self):
        if self._ssh_status['returncode'] == 0:
            print('SSH-Agent status: \t' +
                  self._display_colored_text_(self._GREEN, 'available'))
            if not self._args.short:
                print('Available SSH-Keys:')
                for key in self._ssh_status['keys']:
                    print('\t\t\t' + self._display_colored_text_(self._BLUE, key))
                print('\n')
        else:
            print('SSH-Agent status: \t' +
                  self._display_colored_text_(self._RED, 'not available'))
    ...
    def print_git_status(self):
        if self._git_status['returncode'] == 0:
            ...
            if not self._args.short:
                print('Git config:')
                print('\tName: \t\t' +
                      self._display_colored_text_(self._BLUE, self._git_status['name']))
                print('\tEmail: \t\t' +
                      self._display_colored_text_(self._BLUE, self._git_status['mail']))
                print('\tGPG signingkey: ' +
                      self._display_colored_text_(self._BLUE, self._git_status['gpg-key']))
                print('\n')
        else:
            print('Git-Config status: \t' +
                  self._display_colored_text_(self._RED, 'not available'))
    ...
    def _get_ssh_status_(self):
        ...
        ssh_add_result = subprocess.run(command,
                                        shell=True,
    ...
    def _get_gpg_status_(self):
        gpg_status = {'returncode': None,
                      'keys': []}
    
        subprocess.run(['gpg', '--list-secret-keys'],
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL)
    
        command = ['gpg', '--list-secret-keys']
        gpg_list_result = subprocess.run(command,
                                         stdout=subprocess.PIPE,
                                         stderr=subprocess.PIPE)
        ...
        if gpg_returncode == 0:
            ...
            for key in keys:
                key_info = ''.join(key[0].strip() + ': ' +
                                   key[1].strip() + ' - ' + key[2].strip())
                gpg_status['keys'].append(key_info)
    
            gpg_recipient = re.findall(
                r'.*<([a-zA-Z0-9.!#$%&\'*+=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]'
                r'{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}'
                r'[a-zA-Z0-9])?)*)>.*', gpg_keys_string)
            if len(gpg_recipient) > 0:
                command = ['(set -o pipefail && echo "test" | gpg --sign --recipient ' + gpg_recipient[0] + ')']
                gpg_encrypt_result = subprocess.run(command,
                                                    stdout=subprocess.PIPE,
                                                    stderr=subprocess.PIPE,
                                                    shell=True)
    ...
    def _get_gopass_status_(self):
        ...
        gopass_add_result = subprocess.run(command,
                                           stdout=subprocess.PIPE,
                                           stderr=subprocess.PIPE)
